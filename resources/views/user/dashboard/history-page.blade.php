...
<div class="p-6 rounded-xl">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">Daftar pesanan yang pernah Anda buat. Klik pesanan untuk melihat detail.</p>
        </div>
                ...
                <div class="border rounded-lg overflow-hidden">
                    ...
                    <div class="bg-gray-50 px-5 py-3 flex flex-wrap items-center justify-between gap-3">
                        <div class="flex items-center gap-4">
                            <div>
                                ...
                                <span class="text-xs text-gray-500 ml-2">{{ optional($order->created_at)->format('d M Y, H:i') }}</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusColors[$order->status->value] ?? 'bg-gray-100' }}">
                                {{ $order->status->getLabel() }}
                            </span>
                            <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusColors[$order->payment_status->value] ?? 'bg-gray-100' }}">
                    ...
                    <div class="bg-gray-50 px-5 py-3 flex flex-wrap items-center justify-between gap-3 border-t">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-base font-semibold">Rp {{ number_format((int) $order->total_amount, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
            @empty
                <div class="p-8 text-center border rounded-lg">
                    ...
                    <a href="{{ route('produk.cari') }}" class="inline-block mt-3 text-primary text-sm hover:underline">Mulai Belanja</a>
                </div>
            @endforelse
        </div>
    </div>
