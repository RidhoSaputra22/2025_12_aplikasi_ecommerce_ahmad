...
$trackingSteps = [
    ['key' => 'paid', 'label' => 'Dibayar', 'icon' => 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z'],
    ['key' => 'processing', 'label' => 'Dikemas', 'icon' => 'M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z'],
    ...
    ['key' => 'delivered', 'label' => 'Diterima', 'icon' => 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
];

/**
 * Determine the current step index based on shipment status
 */
function getTrackingStepIndex($shipment, $orderStatus) {
    if (!$shipment) {
        // No shipment yet - only paid step completed
        return 0;
    }
    return match ($shipment->status->value) {
        'pending' => 1,   // Processing/packaging
...
<div class="p-6 rounded-xl">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">Pantau status pengiriman pesanan Anda secara real-time.</p>
        </div>
        ...
        <div class="space-y-6">
            @forelse ($orders as $order)
                @foreach ($order->orderVendors as $orderVendor)
                    @php
                        ...
                        $currentStep = getTrackingStepIndex($shipment, $order->status);
                        $firstItem = $orderVendor->orderItems->first();
                        $product = $firstItem?->productVariant?->product;
                        $image = $product?->productImages?->first();
                        $totalItems = $orderVendor->orderItems->sum('quantity');
                    @endphp
                    <div class="border rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                        ...
                        <div class="bg-gray-50 px-5 py-3 flex flex-wrap items-center justify-between gap-3 border-b">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-xs text-gray-500">{{ $orderVendor->vendor?->store_name ?? 'Vendor' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                @else
                                    <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800">
                                        Menunggu Pengiriman
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="px-5 pb-4">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">
                                            {{ $totalItems }} barang
                                            @if ($orderVendor->orderItems->count() > 1)
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-3 mt-4 pt-3 border-t">
                                <div class="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                                    ...
                                    @if ($shipment?->delivered_at)
                                        <div class="flex items-center gap-1">
                                            ...
                                            Diterima: {{ $shipment->delivered_at->format('d M Y, H:i') }}
                                        </div>
                                    @endif
                                ...
                                <div class="flex items-center gap-2">
                                    @if ($shipment && $shipment->status->value === 'shipped')
                                        <button type="button"
                                            wire:click="$dispatch('openModal', { component: 'components.shipment-tracking-map', arguments: { shipmentId: {{ $shipment->id }} }, title: 'Peta Pengiriman Live', maxWidth: '5xl' })"
                                            class="px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-medium hover:bg-indigo-700 transition-colors flex items-center gap-1.5">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @empty
                <div class="p-12 text-center border rounded-xl">
                    ...
                    <p class="text-gray-400 text-sm">Pesanan yang sudah dibayar akan muncul di sini.</p>
                    <a href="{{ route('produk.cari') }}"
                       class="inline-block mt-4 text-primary text-sm font-medium hover:underline">Mulai Belanja</a>
                </div>
            @endforelse
        </div>
    </div>
